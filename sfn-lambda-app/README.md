...
```bash
sfn-lambda-app$ sam build --use-container
```
...
```bash
sfn-lambda-app$ sam logs -n StockCheckerFunction --stack-name sfn-lambda-app --tail
```
...
```bash
sfn-lambda-app$ pip install pytest pytest-mock --user
sfn-lambda-app$ python -m pytest tests/ -v
```
